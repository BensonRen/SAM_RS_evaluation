...
def prompt_folder_with_point(mode, max_img=999999):
    save_mask_path = 'DG_road_{}_prompt_save'.format(mode)
...
def prompt_folder_with_multiple_points(mode, num_point_prompt, max_img=999999):
    save_mask_path = 'DG_road_{}_prompt_save_numpoint_{}'.format(mode, num_point_prompt)
    if not os.path.isdir(save_mask_path):
        os.makedirs(save_mask_path)
...
def prompt_folder_with_bbox(mask_folder, bbox_df_file='bbox.csv',
    ...
    for ind, row in tqdm(df.iterrows(), total=df.shape[0]):
        # Get image path
        img_path = os.path.join(img_folder, row['img_name'].replace('mask.png','sat.jpg'))  # Note that DG_road has .jpg for img and .png for mask
        # Make sure this image exist
        if not os.path.exists(img_path):
            print('Warning!!! {} does not exist, bypassing now'.format(img_path))
            continue
...
if __name__ == '__main__':
    ###############################################
    # Prompting the center/random points for the DG_road#
    ###############################################
